...
class MultiscaleMultibranchTCN(nn.Module):
    def __init__(self, cfg):
        ...
        input_size = cfg.params.input_dim
        num_channels = [cfg.params.hidden_dim * len(
            cfg.params.tcn_kernel_resolution) * cfg.params.width_mult] * cfg.params.num_layers
        num_classes = cfg.params.num_classes
        self.kernel_sizes = [e for e in cfg.params.tcn_kernel_resolution]
        dropout = cfg.params.dropout
        relu_type = cfg.params.relu_type  #

        self.num_kernels = len(self.kernel_sizes)

        self.mb_ms_tcn = MultibranchTemporalConvNet(input_size, num_channels, self.kernel_sizes, dropout=dropout, relu_type=relu_type)
...
class MultibranchTemporalConvNet(nn.Module):
    def __init__(self, num_inputs, num_channels, kernel_size, dropout=0.2, relu_type='relu'):
